 ...
 * @author Ludowica
 */
@WebServlet(name = "PayElecWaterServlet", urlPatterns = {"/PayElecWaterServlet"})
public class PayElecWaterServlet extends HttpServlet {
        ...
        if (paymentType.equals("acno")) {
            if (pay.checkPayer(idNo)) {
                if (pay.checkBalance(idNo)) {
                    if (pay.addElecWater(paymentType, amount, idNo, date, nic)) {
