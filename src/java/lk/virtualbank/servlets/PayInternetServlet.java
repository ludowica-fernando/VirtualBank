 ...
 * @author Ludowica
 */
@WebServlet(name = "PayInternetServlet", urlPatterns = {"/PayInternetServlet"})
public class PayInternetServlet extends HttpServlet {

    @Override
    protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        String internettype = request.getParameter("internettype");
        ...
        if (paymentType.equals("acno")) {
            if (pay.checkPayer(idNo)) {
                if (pay.checkBalance(idNo)) {
                    if (pay.addInternet(internettype, amount, idNo, date, nic)) {
